...
// Add takes in the information for a new peer and
// adds it to the database.
func Add(id, ip string, port int) error {
	p := models.Peer{
		ClientId: id,
		Ip:       ip,
		Port:     port,
...
// Get takes in the client ID ("peer id" in BEP 3) and returns
// the corresponding peer.
func Get(id string) (models.Peer, error) {
	ctx := context.Background()
